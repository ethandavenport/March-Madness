...
**Key steps:**
- **Tournament results**: Load tournament results and drop unneeded box score columns, keeping scoring, team IDs, and day numbers. Map DayNum to human-readable rounds (Play-in, Round 1–6) using a custom function that accounts for the 2021 schedule quirk.
- **Seeds and naming**: Strip regional prefixes from seeds to create numeric seeds (1–16) and merge them onto winning and losing teams. Normalize ordering by renaming the alphabetically first team as ATeam and the other as BTeam.
...
The final merged DataFrame includes 100+ columns per game, with consistent A/B feature pairs that maintain symmetry between teams.
...
- **BADJ EM** – Barttorvik-style adjusted efficiency margin
- **BADJ O** – Barttorvik adjusted offensive efficiency
- **BARTHAG** – Barttorvik's overall power rating
- **ELITE SOS** – Strength-of-schedule metric emphasizing games against elite opponents
- **KADJ EM** – KenPom adjusted efficiency margin
- **KADJ O** – KenPom adjusted offensive efficiency
- **KPI #** – KPI ranking value
- **Q1 PLUS Q2 W** – Total wins in Quadrant 1 and Quadrant 2 games
- **Q1 W** – Wins specifically against top-tier (Quadrant 1) opponents
- **R SCORE** – Resume score index
- **TALENT** – Talent rating based on recruiting or roster quality
- **WAB** – "Wins Above Bubble"

The selected features are almost all efficiency, schedule-strength, and resume metrics. **Seed is redundant and gets dropped** — the model keeps these broad "catch-all" indicators and discards more niche stats like shooting splits, tempo, height, rebounding, and free throw percentage, implying that their effects are largely absorbed by the higher-level composite metrics.
...
**Elastic Net** was ultimately chosen for deployment because it provided the best balance of accuracy, stability, and bracket realism. The neural network underperformed (no surprise for such a small dataset). LASSO was marginally more accurate but noticeably more volatile and prone to extreme probabilities. Elastic Net delivered competitive log loss while keeping predictions close to the seed-based baseline.

Some out-of-sample predictions from other models were clearly too aggressive:
- LASSO assigned 5-seed Colorado a **15.7%** win probability against 12-seed Georgetown (2021 R1)
- LASSO gave 2-seed Louisville only a **25.4%** chance to beat 7-seed Michigan (2017 R2)
- Boosting placed 1-seed Arizona at **31.4%** to beat 5-seed Houston (2022 S16)
- Boosting rated 3-seed Creighton at **29.7%** to defeat 11-seed Oregon (2024 R2)
...
- Fit a simple logistic model using only the log ratio of seeds to predict win probability
- Use this to estimate typical win probabilities for every 1–16 vs. 1–16 pairing
...
- In a future tournament, whenever a matchup's lift for the worse seed is below the relevant cutoff, the bracket intentionally picks the upset
...
"""
    st.markdown(_readme_md, unsafe_allow_html=True)